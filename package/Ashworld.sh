...
GAMEDIR="/${directory#/}/ports/ashworld"
java_runtime="zulu17.54.21-ca-jre17.0.13-linux"
jar_filename="ashworld.dat"

cd "$GAMEDIR" || { pm_message "Ashworld: game folder missing."; pm_finish; exit 1; }
...
[ -f "$GAMEDIR/$jar_filename" ] || fail "Copy your owned ashworld.dat to ashworld/ashworld.dat."
...
"$JAVA_HOME/bin/java" -Xmx64m -cp runtime/ashworld-host.jar org.portmaster.ashworld.VerifyGame "$GAMEDIR/$jar_filename" || fail "Unsupported or damaged game JAR. Check the README checksum."
source "$GAMEDIR/display.inc" || fail "Display helper missing."
ashworld_display_setup || fail "Use auto or WIDTHxHEIGHT in resolution.txt."
printf 'Firmware: %s; display: %s\n' "$CFW_NAME" "$ashworld_display_description"
export SDL_GAMECONTROLLERCONFIG="$sdl_controllerconfig"
export HOTKEY=back
$GPTOKEYB2 java -c "$GAMEDIR/ashworld.ini" &
mapper_pid=$!
...
$ESUDO env "${display_env[@]}" "$weston_dir/westonwrap.sh" headless noop kiosk crusty_glx_gl4es \
  ...
  "-Dashworld.dat=$GAMEDIR/$jar_filename" "-Dashworld.saves=$SAVEDIR" \
  -Dashworld.fullscreen=true "${display_java[@]}" \
  -cp "$GAMEDIR/runtime/ashworld-host.jar:$GAMEDIR/$jar_filename" org.portmaster.ashworld.Main
