...
public class Main extends myCanvas {
    ...
    public static Lwjgl3ApplicationConfiguration configuration() throws Exception {
        int width = Integer.getInteger("ashworld.width",640), height = Integer.getInteger("ashworld.height",480);
        ...
        Path saves = Paths.get(System.getProperty("ashworld.saves","saves")).toAbsolutePath();
        java.nio.file.Files.createDirectories(saves);
        Lwjgl3ApplicationConfiguration cfg = new Lwjgl3ApplicationConfiguration();
        cfg.setTitle("Ashworld"); cfg.setWindowedMode(width,height);
        cfg.setHdpiMode(HdpiMode.Pixels); cfg.setResizable(false);
        cfg.setForegroundFPS(60); cfg.setIdleFPS(30); cfg.useVsync(false);
        java.nio.file.Files.createDirectories(saves.resolve("legacy-preferences"));
        cfg.setPreferencesConfig(saves.resolve("legacy-preferences").toString(), com.badlogic.gdx.Files.FileType.Absolute);
        cfg.setOpenGLEmulation(Lwjgl3ApplicationConfiguration.GLEmulation.GL20,2,0);
        cfg.disableAudio(Boolean.getBoolean("ashworld.noAudio"));
        cfg.setInitialVisible(!Boolean.getBoolean("ashworld.hidden"));
        if (Boolean.getBoolean("ashworld.fullscreen")) cfg.setFullscreenMode(Lwjgl3ApplicationConfiguration.getDisplayMode());
        return cfg;
    }

    public static void main(String[] args) throws Exception {
        VerifyGame.check(Paths.get(System.getProperty("ashworld.dat","ashworld.dat")));
        System.out.println("Ashworld PortMaster host 0.1.0 | " + System.getProperty("os.arch") + " | 60 updates/s");
    ...
    @Override public void create() {
        ...
        myPreferences = new com.orangepixel.ashworld.desktop.DesktopPreferences() {
            @Override public String getDefaultPreferencesDirectory() { return System.getProperty("ashworld.saves","saves"); }
        ...
        myServerData = (com.orangepixel.utils.ServerData)Proxy.newProxyInstance(Main.class.getClassLoader(),
            new Class<?>[]{com.orangepixel.utils.ServerData.class},(self,method,args)-> {
                if (method.getReturnType() == boolean.class) return false;
                if (method.getReturnType() == int.class) return 0;
                if (method.getReturnType() == String.class) return "Offline handheld port";
                return null;
            });
        ...
        System.out.println("GAME_CREATE_OK offline; PortMaster keyboard input; saves=" + System.getProperty("ashworld.saves","saves"));
    }
    ...
    @Override public void render() {
        ...
        super.render();
        frames++;
        int smoke = Integer.getInteger("ashworld.smokeFrames",0);
        if (smoke > 0 && frames >= smoke) {
            String capture = System.getProperty("ashworld.capture");
    ...
    @Override public void pause() { if (activePlayer != null) activePlayer.saveSettings(PROFILEID); super.pause(); }
    @Override public void resume() { lastFrame = 0; super.resume(); }
    @Override public void dispose() {
        if (activePlayer != null) activePlayer.saveSettings(PROFILEID);
