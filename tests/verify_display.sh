...
for size in 640x480 720x480 720x720 1024x768 1280x720 960x544 320x240; do
    ...
    [[ "${display_java[0]}" == "-Dashworld.width=$DISPLAY_WIDTH" ]]
    [[ "${display_java[1]}" == "-Dashworld.height=$DISPLAY_HEIGHT" ]]
done
DISPLAY_WIDTH=0 DISPLAY_HEIGHT=0
ashworld_display_setup
[[ "${#display_env[@]}" == 0 && "${#display_java[@]}" == 0 ]]
for bad in 0x480 640x0 100x100 9999x720 640x480oops '640x480;exit' 640X480; do
    ASHWORLD_RESOLUTION="$bad"
    if ashworld_display_setup; then echo "Accepted invalid size: $bad"; exit 1; fi
done
ASHWORLD_RESOLUTION=0720x0480
ashworld_display_setup
[[ "${display_java[0]}" == '-Dashworld.width=720' ]]
[[ "${display_java[1]}" == '-Dashworld.height=480' ]]
printf '720x720\r\n' > "$GAMEDIR/resolution.txt"
ASHWORLD_RESOLUTION=""
ashworld_display_setup
[[ "${display_java[1]}" == '-Dashworld.height=720' ]]
ASHWORLD_RESOLUTION=1280x720
ashworld_display_setup
[[ "${display_java[0]}" == '-Dashworld.width=1280' ]]
